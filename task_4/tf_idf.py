...
def prepare():
    """
    Загружает библиотеку неважных слов
    """
...
def is_word(word):
    """
    Строка считается словом, если она не входит в список неважных и содержит только кириллицу
    """
...
def tokenize(html_text, lemmatized=False):
    """
    Токенизация и лемматизация контента html
    """
    parser = 'html.parser'
...
def process(zip_file, lemmas_path, tokens_path):
    ...
    for (index, file) in enumerate(zip_file.filelist):
        ...
        with open(f"token_results/{index}.txt", "w", encoding="utf8") as token_f:
            token_f.write("\n".join(result_tokens))

        with open(f"lemma_results/{index}.txt", "w", encoding="utf8") as lemma_f:
            lemma_f.write("\n".join(result_lemmas))
...
if __name__ == '__main__':
    ...
    lemmas_index_path = "../task_3/index.txt"
    tokens_index_path = "../task_3/index_tokens.txt"
